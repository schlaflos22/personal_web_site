...
//Загрузка файла поcтера в директорию img
    if(!empty($_FILES['movie_poster'] && !$_FILES['movie_poster']['error'])) {
        ...
        if (copy($_FILES['movie_poster']['tmp_name'],$upload_path)) {
            ...
            echo "Файл постера корректен и был успешно загружен.\n";
        }else {
            echo "Файл постера не был загружен\n";
        }
    }
    ...
    for( $i =0;$i< count($_FILES['files']['name']);$i++) {
       ...
       if(copy($userFileTmp,  $uploadPath)){
            echo "Файлы картинок для галереи корректны и были успешно загружены.";
        }else{
            echo "Возможная ошибка при загрузке Файлов картинок для галереи.";
        }
    }
}
?>
